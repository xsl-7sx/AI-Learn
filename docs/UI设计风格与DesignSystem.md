# 知练（KnowPractice）— UI 设计风格与 Design System

> **产品品牌：** 知练（KnowPractice）。对外主名「知练」；「闯关」为练测单元交互说法。
...
**一句话：** 用 **图文笔记式 Bento** 呈现 Duolingo 式关卡练测，气质专业克制、偏学习工具而非重度娱乐漫画。
...
| 5b | 金币 / XP | **原型已移除**顶栏金币、反馈「+10 金币」、报告「+20 XP」 |
| 6 | 正误色 | 文档/原型可用漫画微调色；**实现**对齐 `#07c160` / `#fa5151` |
| 7 | 单题计时 | MVP **不做** |
| 8 | 生命值/灵韵 | MVP **不做** |
| 9 | 扩展页保真 | 中保真（同风格，Mock 可简化） |
| 10 | HTML 原型 | **已完成** 22 屏 gallery（MVP 9 + 扩展 6 + 报告 7）；见 [实施指引](./UI-HTML原型实施指引.md) |
...
| 进度条 | `width` 平滑过渡（0.35s） | — | 暖橙双色渐变；生成中由 `loading-steps.js` 驱动；**无**彩虹流光 |
...
| `bt-progress-line` | 进度条（8px 圆角；主色 `#F06A2A→#FFB380`；对手 `--rival` 蓝） |
| `bt-opt-check` | 答对/答错圆章标记 |
| `bt-input-icon` | 输入框尾标（铅笔） |
