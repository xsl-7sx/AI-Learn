# 知练（KnowPractice）— 微信小程序方案设计文档

> **产品品牌：** 知练（KnowPractice）。本文档文件名保留历史项目称谓；对外、小程序与 UI 统一使用「知练」。
...
泛知识时代，用户面对长篇推文、报告或学习资料时，被动阅读易疲劳，且缺乏即时反馈——「收藏从未停止，阅读从未开始」。**知练**以**以测代学**为核心理念：用户输入学习目标，AI 自动生成交互式练测题目，用户在答题中获得即时反馈，完成后获得 AI 复盘报告，从而更轻松地掌握任意知识（覆盖校园自学、职考与企业内训场景）。
...
§6 页面结构以方案为准；UI 文档在屏数上补充 **结算 skeleton 独立态（第 9 屏）** 及扩展/报告示意屏。视觉层以 **`bento-theme.css`（Bento 暖橙）+ `paper-lines-*.svg`** 为准；保留水彩横纹，**无方格格纹、无顶栏金币/反馈金币/报告 XP 示意、无书本外壳**；主导航为 **`bt-tabbar-float` 悬浮胶囊**（机框内、屏外，底区纸纹与内容区连续）；屏内默认顶对齐，仅生成中页垂直居中。
