...
| 首页滚动 | 顶栏 `ref-topbar` 固定；`ref-home-scroll` 单区滚动；底栏不被裁切 | ✅ v1.7.3 |
| 热门主题 | `.ref-topic-strip` 横滑 + 边缘渐变；`ref-home-topics.js` 拖拽/滚轮/换一批 | ✅ v1.7.3 |
...
| # | 屏 | 要点 | 状态 |
| --- | -- | ---- | ---- |
| 1 | 首页 | `ref-home`：顶栏固定；compose 输入+示例；热门主题横滑（5 卡/组、边缘渐变）；连胜；未完成「继续」 | ✅ |
...
| ~~P2~~ | ~~热门主题横滑交互~~ | ✅ v1.7.3：`.ref-topic-strip` 拖拽/滚轮/键盘；屏内隐藏滚动条 |
| P3 | 深色模式 | 当前仅浅色 Bento |
