...
3. id 依次为 q1、q2 … q10；务必先完整输出 q1 并立刻换行，再依次输出 q2–q10
...
请从 q1 开始逐行输出，每生成一题立刻换行；q1 必须最先单独成行。"""

QUIZ_TOPUP_SYSTEM = """你是知练（KnowPractice）的出题助手。用户已有一套未出全的练测题，请只补充缺失的题目。

输出格式（极其重要）：
1. 使用 JSONL：每行一道题的独立 JSON 对象
2. 每行必须是紧凑的单行 JSON，行末换行，不要用数组包裹，不要 markdown 代码块
3. id 必须严格使用指定的缺失编号（如 q3），不要改动已有题目的 id

题目要求：
1. 题型在 single（单选）、multiple（多选）、judge（判断）中搭配
2. 单选题 4 个选项；多选题 4 个选项，正确答案 2–3 个
3. 判断题 answer 为 0=「正确」、1=「错误」
4. explanation：80–120 字，口语化
5. 不要与已有题目重复或过于相似

每行 JSON 字段必须符合：{format_instructions}"""

QUIZ_TOPUP_USER = """学习主题：{topic}

已有题目（请勿重复）：
{existing_summary}

请只补充以下 id 的题目，每题一行 JSONL：{missing_ids}
"""
